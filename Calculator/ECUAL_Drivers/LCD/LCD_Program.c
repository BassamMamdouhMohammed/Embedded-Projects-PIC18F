...
STD_RETURN_t ECUAL_LCD_8BIT_Write_Custome_Char(const LCD_8BIT_t *_LCD,uint8 Row,uint8 Column,uint8 Mem_Pos,const uint8 _chr[]){
    ...
    uint8 Index_8Bit = ZERO_Init;
        if(NULL == _LCD){
            ...
            for(Index_8Bit =0; Index_8Bit <= 7; Index_8Bit++){
...
STD_RETURN_t ECUAL_LCD_4BIT_Init(const LCD_4BIT_t *_LCD){
    STD_RETURN_t Return_Status = STD_OK;
    uint8 Index_4Bit = ZERO_Init;
        if(NULL == _LCD){
            ...
            __delay_ms(20);
            Return_Status = ECUAL_LCD_4BIT_Write_Command(_LCD,_LCD_8BIT_BUS_MODE_2LINE);
            __delay_ms(5);
            Return_Status = ECUAL_LCD_4BIT_Write_Command(_LCD,_LCD_8BIT_BUS_MODE_2LINE);
            __delay_ms(150);
            Return_Status = ECUAL_LCD_4BIT_Write_Command(_LCD,_LCD_8BIT_BUS_MODE_2LINE);
...
STD_RETURN_t ECUAL_LCD_4BIT_Write_String_Position(const LCD_4BIT_t *_LCD,uint8 *_Str,uint8 _Row, uint8 _Column){
    STD_RETURN_t Return_Status = STD_OK;
        if(NULL == _LCD){
            Return_Status = STD_NOT_OK;
        }
            ...
            Return_Status  = LCD_4Bit_Set_Cursor(_LCD,_Row,_Column);
            while(*_Str){
                ECUAL_LCD_4BIT_Write_Data(_LCD,*_Str++);
            }
        }
    return Return_Status;
}
STD_RETURN_t ECUAL_LCD_4BIT_Write_Custome_Char(const LCD_4BIT_t *_LCD,uint8 Row,uint8 Column,uint8 Mem_Pos,const uint8 _chr[]){
    ...
    uint8 Index_4Bit = ZERO_Init;
        if(NULL == _LCD){
            ...
            for(Index_4Bit =0; Index_4Bit <= 7; Index_4Bit++){
