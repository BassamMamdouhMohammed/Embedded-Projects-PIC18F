...
STD_RETURN_t ECUAL_KEYPAD_Init(const KEYPAD_t *_Keypad) {
...
STD_RETURN_t ECUAL_KEYPAD_GetValue(const KEYPAD_t *_Keypad, uint8 *value) {
    STD_RETURN_t Return_Status = STD_OK;
    uint8 Row_Index = ZERO_Init;
    uint8 Column_Index = ZERO_Init;
    ...
    if (NULL == _Keypad) {
        ...
        for (Row_Index = ZERO_Init; Row_Index < _KEYPAD_ROWS; Row_Index++) {
            for (l_counter = ZERO_Init; l_counter < _KEYPAD_ROWS; l_counter++) {
                MCAL_GPIO_Pin_Write_Logic(&(_Keypad->Rows_Pin[l_counter]), GPIO_LOW);
            }

            MCAL_GPIO_Pin_Write_Logic(&(_Keypad->Rows_Pin[Row_Index]), GPIO_HIGH);
            __delay_ms(50);

            for (Column_Index = ZERO_Init; Column_Index < _KEYPAD_COLUMNS; Column_Index++) {
                MCAL_GPIO_Pin_Read_Logic(&(_Keypad->Columns_Pin[Column_Index]), &column_logic);
                if (GPIO_HIGH == column_logic) {
                    *value = btn_values[Row_Index][Column_Index];
                }
            }
        }
    }
