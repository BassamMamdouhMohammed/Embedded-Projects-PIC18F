 ...
 * Description: Temperature Meter Logic main code
 * Version    : 1.0.0
 */
...
ADC_t LM35 = {
    ...
    .ADC_InterruptHandler = ADC_Update,
};
...
void ADC_Update(void)
{
    temperature =  MCAL_ADC_GetConversionResult(&LM35) * division;
}
...
STD_RETURN_t Convert_U16toString(uint16 val, uint8 *str){
    ...
    if(NULL != str){
        memset(str, ' ', 5);
        str[5] = '\0';
        sprintf((char *)Temp_String, "%i", val);
