 ...
 * Description : Implementation of Timer0 functions and ISR 
 */
...
static uint16 TMR0_Preload = ZERO_Init;

static inline void TIM0_SelectMode(const TIMER0_t *_TMR);
static inline void TIM0_SelectControlBitSize(const TIMER0_t *_TMR);
static inline void TIM0_SelectPrescaler(const TIMER0_t *_TMR);

STD_RETURN_t MCAL_TIM0_Start(const TIMER0_t *_TMR){
    STD_RETURN_t Return_Status = STD_OK;
        if(NULL == _TMR){
            ...
            TIMER0_STOP();
            TIM0_SelectPrescaler(_TMR);
            TIM0_SelectMode(_TMR);
            TIM0_SelectControlBitSize(_TMR);
...
STD_RETURN_t MCAL_TIM0_Stop(const TIMER0_t *_TMR){
    STD_RETURN_t Return_Status = STD_OK;
        if(NULL == _TMR){
            Return_Status = STD_NOT_OK;
        }
...
STD_RETURN_t MCAL_TIM0_Set(const TIMER0_t *_TMR, uint16 _Value){
    STD_RETURN_t Return_Status = STD_OK;
        if(NULL == _TMR){
            Return_Status = STD_NOT_OK;
        }
...
STD_RETURN_t MCAL_TIM0_Get(const TIMER0_t *_TMR, uint16 *_Value){
    STD_RETURN_t Return_Status = STD_OK;
